...
public class Album {
    ...
    private final static String       endpoint = "https://api.spotify.com/v1/albums/"; //Local Endpoint
     ...
     *                               considered available in a market when at least 1 of its tracks is available in
     *                               that market.
     ...
     *                               string.
     * @param popularity             The popularity of the album. The value will be between 0 and 100, with 100 being
     *                               the most popular. The
     *                               popularity is calculated from the popularity of the album’s individual tracks.
     * @param release_date_precision The date the album was first released, for example "1981-12-15". Depending on the
     *                               precision, it might be
     *                               shown as "1981" or "1981-12".
     * @param restrictions           The precision with which release_date value is known: "year" , "month" , or
     *                               "day".
    ...
    public Album requestAlbum(String ID, String market, AccessToken at) throws IOException {
        OkHttpClient client = new OkHttpClient();
        Gson         gson   = new Gson();
        Request request = new Request.Builder().url(getEndpoint() + ID + "?market=" + market).get()
                                               .addHeader("Accept",
                                                       "application/json")
                                               .addHeader("Content-Type", "application/json")
